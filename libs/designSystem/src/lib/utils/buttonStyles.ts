import { ButtonVariantType, ButtonSizeType } from '../types/button.types';
  ...
  switch (variant) {
    case 'primary':
      return `
        ...
        ${
          visuallyDisabled
            ? 'bg-[#F9FAFB] disabled:text-[#98A2B3] disabled:border disabled:border-[#EAECF0]'
            : ''
        }
      `;
...
export const getButtonSizeClasses = (size: ButtonSizeType = 'md') => {
  switch (size) {
    ...
    case 'lg':
      return 'text-base px-5 py-2.5 h-10';
    case 'xl':
      return 'text-base px-6 py-3 h-11';
    case '2xl':
      return 'text-lg px-7 py-3.5 h-12';
    default:
      return 'text-sm px-4 py-2 h-9';
  }
};
